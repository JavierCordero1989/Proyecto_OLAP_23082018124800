...
@section('content')
    <div id="app-vue-reports" class="content">
        <div class="jumbotron">
            <div class="col-md-12">
                {!! Form::button('<i class="fas fa-brush"></i> Limpiar...', ['class'=>'btn btn-default pull-left', 'v-on:click'=>'cleanInputs']) !!}
            </div>
            ...
            <div class="form-group col-md-3">
                ...
                <select class="form-control" v-model="area" @change="cargarDisciplinas(area)" multiple="multiple">
                    <option disabled value="">- - - ÁREAS - - -</option>
                    <option class="text-uppercase" v-for="(item, index) in lista_areas" :value="index">@{{ item }}</option>
                </select>
            </div>
            ...
            <div class="form-group col-md-3">
                ...
                <select v-model="disciplina" class="form-control" multiple="multiple">
                    <option disabled value="">- - - DISCIPLINAS - - -</option>
                    <option v-for="(item, index) in lista_disciplinas" :value="item">
                        @{{ item }}
                    </option>
                </select>
                <pre>
