            ...
            <div class="box-header">
                <div class="box-body">
                    @foreach($usuarios as $supervisor)
                        <div class="hide">
                            {!! $supervisor->hasRole(['Super Admin', 'Supervisor 1']) !!}
                        </div>
                    @endforeach
...
@section('scripts')
    ...
    <script>
        let lista = <?php echo json_encode($usuarios); ?>
        ...
        new Vue({
            ...
            data: {
                ...
                lista_supervisores: lista
            },
            methods: {
                getDeleteRoute: function(id) {
                    let route = '{{route("supervisores.destroy", ":id")}}'
                    route = route.replace(":id", id)
                    return route
                },
                getEditRoute: function(id) {
                    let route = '{{route("supervisores.edit", ":id")}}'
                    route = route.replace(":id", id)
                    return route
                },
            ...
            computed: {
                listaFiltro() {
                    let filtro = this.lista_supervisores
